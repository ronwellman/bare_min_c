...
\begin{document}
	...
	If you've never looked at assembly code before, this can look a bit crazy.  Without going into the nitty gritty of assembly\index{assembly} language syntax, I wanted to take a second and point out a few things even though this isn't a very good example to use anyway.  There is something called a \textbf{stack frame}\index{stack frame}.  \textit{Stack frames} typically hold the variables declared within a function to include return addresses from the calling function.  Each function gets its own stack frame.  There is also something called a \textbf{call stack}\index{call stack} that is composed of various stack frames and the return addresses.  It might look something like this:
	...
	\subsection{Heap}
	The \textbf{heap}\index{heap} is a section of memory that allows for the dynamic allocation of memory and for the establishment of \textit{global} variables.  Dynamic memory does not get automatically cleaned up when a function returns.  Instead, the programmer has to keep track of it and clean it up manually as needed.  If not, memory will be lost (leaked).  The four most common functions you'll see when working with dynamic memory are:
	...
	The point of the \textit{getInput} function is to prompt the user for input and take whatever the user types and store it into a buffer.  Its not particularly exciting code but will continually allocate blocks of 16 bytes until the user hits ENTER or send an End of File (EOF).  This code was meant to demonstrate the use of \textit{malloc}, \textit{realloc}, and \textit{free} and was not meant to be efficient or even practical for that matter.  
